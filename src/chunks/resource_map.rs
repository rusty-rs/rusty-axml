...
//! AXML resource maps
//!
//! The resource map contains an array of `u32` mapping strings in the
//! string pool back to resource identifiers. Is it optional.
...
/// Get the attribute name given its index
fn get_resource_string(id: &u32) -> Option<String> {
    // For now, we only care about the attribute names.
    let id = id - 0x1010000;

    ATTR_NAMES.get(id as usize).map(|attr| attr.to_string())
}
